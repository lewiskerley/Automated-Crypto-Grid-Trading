...
file_path_template = 'classical_interval_{}.csv'
...
def calculate_metrics(df):
    ...
    return roi, mdd, volatility, sharpe_ratio, count
...
for interval in range(100, 2500, 100):
    file_path = file_path_template.format(interval)
    if os.path.exists(file_path):
        df = pd.read_csv(file_path)
        roi, mdd, volatility, sharpe_ratio, count = calculate_metrics(df)
        
        # Store the results in the dictionary
        metrics['Interval'].append(interval)

...
X = data.iloc[:, 1:-1].values # data
y = data['grid_result'].astype(int).values # result
...
def train_ann():
    ...
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy']) # binary cross entropy loss function
    model.fit(X_train, y_train, batch_size=1, epochs=500) # batch size 1, epochs 500
    ...
    y_pred_binary = (y_pred > 0.5).astype(int) # threshold value for prediction
    ...
    # testing different threshold values for optimisation
    y_pred_binary = (y_pred > 0.7).astype(int)
    print("Confusion Matrix 0.7:")
    print(confusion_matrix(y_test, y_pred_binary))
    print("\nClassification Report:")
    print(classification_report(y_test, y_pred_binary))
    y_pred_binary = (y_pred > 0.9).astype(int)
    print("Confusion Matrix 0.9:")
    print(confusion_matrix(y_test, y_pred_binary))
    print("\nClassification Report:")
    print(classification_report(y_test, y_pred_binary))
    ...
    model.save('grid_trading_ann_model.keras') # save the model
...
def ann_grid_trader():
    ...
    model = load_model('grid_trading_ann_model.keras')
    ...
    for i in range(0, len(agg_data) - 5):
        ...
        if (Y_prediction[i] > 0.7): # optimal threshold: 0.7
    ...
    results_csv.to_csv("ann_results.csv", index=False)
